...
public partial class MudThemeManager : ComponentBaseWithState
{
    ...
    public ThemePreset? SelectedThemePreset { get; set; }
    ...
    private async Task ApplyThemePreset()
    {
        ...
        {
            var result = await DialogService.ShowMessageBox("Notice",
                $"Are you sure you want to override the current theme with the {SelectedThemePreset.PresetName} theme?",
                "Apply Theme", cancelText: "Cancel");
    ...
    {
        ToggleExportDialog();
        _jsonExport = JsonSerializer.Serialize(_customTheme);
    }

    private async Task RevertTheme()
    {
        var result = await DialogService.ShowMessageBox("Notice",
            "Are you sure you want to revert the theme to the default values?", "Revert Changes",
            cancelText: "Cancel");
        if (result.HasValue && result.Value)
        {
            Theme = new MudTheme();
            _customTheme = new MudTheme();
            _currentPaletteLight = new PaletteLight();
            _currentPaletteDark = new PaletteDark();
            _currentPalette = GetPalette();
            StateHasChanged();
        }
    }
    ...
    private async Task ImportTheme(IBrowserFile file)
    {
        try
        {
            ...
            {
                var result = await DialogService.ShowMessageBox("Notice",
                    "Are you sure you want to override the current theme with the imported one?",
                    "Import Theme", cancelText: "Cancel");
                if (result.HasValue && result.Value)
                {
    ...
    public Task UpdatePalette(ThemeUpdatedValue value)
    {
        ...
        if (Theme is null || _customTheme is null) return Task.CompletedTask;
    ...
    public static string GenerateCSharpCode(object obj, string varName = "obj", int indentLevel = 0)
    {
        ...
        foreach (var prop in type.GetProperties(BindingFlags.Public | BindingFlags.Instance))
        {
            ...
            var valueStr = FormatValue(value, prop.PropertyType, indentLevel + 1);
        ...
        if (typeof(IEnumerable).IsAssignableFrom(type) && type != typeof(string))
        {
            ...
            foreach (var item in (IEnumerable)value)
                sb.AppendLine(new string(' ', (indentLevel + 1) * 4) +
                              FormatValue(item, item.GetType(), indentLevel + 1) + ",");
    ...
    {
        if (_customTheme is not null) UpdateCustomTheme();
    }
    ...
    private Task OnDefaultBorderRadiusAsync(int value)
    {
        ...
        if (_customTheme is null) return Task.CompletedTask;
    ...
    {
        if (_customTheme is null) return;

        if (_currentPaletteLight is not null) _customTheme.PaletteLight = _currentPaletteLight;

        if (_currentPaletteDark is not null) _customTheme.PaletteDark = _currentPaletteDark;
